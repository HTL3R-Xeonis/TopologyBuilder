...
# verification (verify=False in _upload_disks) - the same choice
# ESXiConnection.connect already makes via its own SSLContext. Without this,
# urllib3 warns on every single upload request about a choice that was made
...
class OVAImporter:
    ...
    def import_ova(
        ...
        :param ova_path: local filesystem path to the .ova file
    ...
    def _wait_for_lease_ready(lease: vim.HttpNfcLease) -> None:
        ...
        :return:
        :raises RuntimeError: Is thrown when the lease fails to become ready.
        """
        while lease.state == vim.HttpNfcLease.State.initializing:
            time.sleep(_LEASE_POLL_INTERVAL_SECONDS)
        if lease.state != vim.HttpNfcLease.State.ready:
            logger.error(msg := f"HTTP NFC lease failed to become ready: {lease.state}")
            raise RuntimeError(msg)
    ...
    def _upload_disks(
        ...
        with requests.Session() as session:
            ...
            for file_item in file_items:
                ...
                upload_url = device_url.url.replace("*", self.esxi_connection.ip)
                ...
                # Content-Length header makes it ALSO add
                # 'Transfer-Encoding: chunked' on top of that Content-Length,
                # producing a self-contradictory request. ESXi's streamVmdk
                # endpoint doesn't dechunk it and instead reads the
                # chunk-framing bytes as VMDK data, failing immediately. A
                # seekable file-like object lets requests determine the real
                # length itself (via seek/tell) and send a plain, non-chunked
                # body - still streamed internally, never buffered fully in
                # memory.
