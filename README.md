...
| `topologybuilder verify --address ... --esxi_username ...` | Structural health check against a deployed topology (GNS3 nodes started, ESXi VMs powered on, trunk NIC wiring, port groups present). Not a ping test - this project never assigns IP addresses to nodes, so there's no address to ping; an ESXi VM's IP (if VMware Tools reports one) is shown for information only and never fails the check, since topology VLANs have no DHCP server. |
